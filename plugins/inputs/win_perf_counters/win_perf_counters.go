...
func extractObjectInstanceCounterFromQuery(query string) (computer string, object string, instance string, counter string, err error) {
...
func (m *Win_PerfCounters) AddItem(counterPath string, instance string, measurement string, includeTotal bool) error {
	...
	for _, counterPath := range counters {
		...
		parserComputer, parsedObjectName, parsedInstance, parsedCounter, err := extractObjectInstanceCounterFromQuery(counterPath)
		if err != nil {
			return err
		}
		...
		newItem := &counter{counterPath, parserComputer, parsedObjectName, parsedCounter, parsedInstance, measurement,
		...
		for _, PerfObject := range m.Object {
			for _, counter := range PerfObject.Counters {
				for _, instance := range PerfObject.Instances {
					...
					if instance == "------" {
						counterPath = computer + "\\" + objectname + "\\" + counter
					} else {
						counterPath = computer + "\\" + objectname + "(" + instance + ")\\" + counter
					}
